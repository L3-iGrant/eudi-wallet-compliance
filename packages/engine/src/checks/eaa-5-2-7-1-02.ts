...
export async function check(
  ...
  if (typeof value !== 'number' || !Number.isInteger(value) || value < 0) {
    return {
      ...
      status: 'fail',
      evidenceRef: EVIDENCE_REF,
      notes: `nbf is ${JSON.stringify(value)}, not a non-negative integer NumericDate per RFC 7519 §2. Expected a JSON number of seconds since 1970-01-01T00:00:00Z (e.g. 1735689600); string-quoted timestamps and floating-point/negative values are rejected.`,
    };
  }
  return {
    controlId: CONTROL_ID,
